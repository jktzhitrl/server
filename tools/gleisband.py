...
# (Name, km, Art)  Art: bf = von uns geplanter Bahnhof, kb = Kreuzungsbahnhof,
# hp = Haltepunkt, anst = Anschlussstelle, ausgen = nicht Teil von Los 5
STRECKE_1 = [("Erx", 0.0, "ausgen"), ("Krug", 34.5, "ausgen"), ("Zollfurt", 35.5, "hp"),
             ("Waldenberg", 36.5, "bf"), ("Burgwald", 40.0, "hp"), ("Hyxel", 49.0, "bf"),
             ("Obersosa", 63.5, "bf"), ("Feldheim", 75.5, "ausgen"),
             ("Lossow", 88.0, "ausgen"), ("Furth", 117.5, "ausgen")]
STRECKE_2 = [("Bernstein", 0.0, "ausgen"), ("Silberberg", 24.5, "bf"), ("Obersosa", 51.0, "bf")]
STRECKE_3 = [("Waldenberg", 0.0, "bf"), ("Wehrheim", 7.0, "kb"), ("Sandheide", 14.5, "kb"),
             ("Getreidesilo", 18.0, "anst"), ("Wassergrund", 19.0, "kb"), ("Gbf", 29.5, "ausgen")]

W, X0, X1 = 1980, 360, 1930
ROT, TIEF, STAHL, GRAU, SOFT = "#C8102E", "#16202E", "#4A6FA5", "#98A4B4", "#5C6879"
FARBE = {"bf": ROT, "kb": ROT, "hp": TIEF, "anst": STAHL, "ausgen": GRAU}
# Beschriftungsspuren: zwei ueber, zwei unter dem Gleisband
SPUREN = [(-26, -44), (-62, -80), (34, 52), (70, 88)]
...
def band(y, nummer, strecke, angabe, stationen, kmax, linienfarbe):
    ...
    for name, km, art in stationen:
        ...
        if abs(dy_name) > 40:          # zweite Spur braucht eine Fuehrungslinie
            y1 = y + (14 if dy_name > 0 else -14)
            y2 = y + (dy_name - 12 if dy_name > 0 else dy_name + 6)
            o.append(f'<line x1="{x:.1f}" y1="{y1}" x2="{x:.1f}" y2="{y2}" stroke="#C9D2DE" stroke-width="1.5"/>')
        fett = "700" if art in ("bf", "kb") else "400"
        farbe_txt = TIEF if art != "ausgen" else GRAU
        o.append(f'<text x="{x:.1f}" y="{y+dy_name}" font-size="14.5" font-weight="{fett}" '
                 f'fill="{farbe_txt}" text-anchor="middle">{name}</text>')
        o.append(f'<text x="{x:.1f}" y="{y+dy_km}" font-size="11.5" fill="{SOFT}" text-anchor="middle" '
                 f'font-family="Courier New, monospace">km {km_text(km)}</text>')
    return "\n".join(o)


def legende(y):
    eintraege = [("bf", "Bahnhof, von uns geplant"), ("hp", "Haltepunkt"),
                 ("anst", "Anschlussstelle"), ("ausgen", "ausgenommen / nicht Los 5")]
    ...
    for art, text in eintraege:
        ...
        x += 26 + len(text) * 7.0 + 48
    o.append(f'<text x="1300" y="{y+33}" font-size="12.5" fill="{SOFT}" font-style="italic">'
             f'Kilometrierung maßstäblich je Strecke</text>')
    return "\n".join(o)


def build():
    teile = [
        band(210, "Strecke 1", "Krug (ausgen.) – Obersosa – Furth (ausgen.)",
             "Hauptbahn · zweigleisig · 120 km/h · elektrifiziert", STRECKE_1, 117.5, TIEF),
        band(450, "Strecke 2", "Bernstein (ausgen.) – Silberberg – Obersosa",
             "Hauptbahn · zweigleisig · 120 km/h · elektrifiziert", STRECKE_2, 51.0, TIEF),
        band(690, "Strecke 3", "Waldenberg – Gbf (ausgen.)",
             "Nebenbahn · eingleisig · 80 km/h · elektrifiziert", STRECKE_3, 29.5, STAHL),
        legende(838),
    ]
    return (f'<svg id="gleisband" width="{W}" height="900" viewBox="0 0 {W} 900" '
            f'xmlns="http://www.w3.org/2000/svg" font-family="Calibri, Arial, sans-serif">'
            f'<rect width="{W}" height="900" fill="#fff"/>{"".join(teile)}</svg>')
